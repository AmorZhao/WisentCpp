/*
 *  This code is adapted from the BOSS project, including:
 *    https://github.com/symbol-store/BOSS/blob/temp_lazy_loading_compression/Source/Utilities.hpp
 *    https://github.com/symbol-store/BOSS/blob/temp_lazy_loading_compression/Source/ExpressionUtilities.hpp
 *    https://github.com/symbol-store/BOSS/blob/temp_lazy_loading_compression/Source/Expression.hpp
 *  
 *   This section provides:
 *    - ComplexExpression_WithAdditionalCustomAtoms (overall structure)
 *        - {head, static, dynamic, spans}
 *  
 *    - Expression_WithAdditionalCustomAtoms (the core value type)
 *        - bool, int8_t, int16_t, int32_t, int64_t, float_t, double_t, std::string
 *        - Symbol (expression head)
 *        - Span (move-only span of values)
 *        - AdditionalCustomAtoms 
 *  
 *    - ExpressionArguments_WithAdditionalCustomAtoms (nested arguments)
 *        - ArgumentWrapper (type-erased wrapper for arguments)
 *        - ExpressionArguments_WithAdditionalCustomAtoms_Wrapper
 *  
 *    - ExpressionSpanArguments_WithAdditionalCustomAtoms (spans)
 *  
 *   It also includes:
 *    - Type-safe getters
 *    - ExtensibleExpressionSystem
 *    - ExpressionBuilder
 */
...
#pragma region variant_type_traits
...
struct make_variant_members_const;

template <typename... Args> 
struct make_variant_members_const<std::variant<Args...>> {
    using type = std::variant<Args const...>;
};

template <typename T, typename... Args> 
struct variant_amend;
...
template <typename L, typename R> using comparability = decltype(std::declval<L>() == std::declval<R>());
...
#pragma endregion variant_type_traits

namespace boss {

namespace expressions {

#pragma region clone_reason
...
#pragma endregion clone_reason

// bool, std::int8_t, std::int16_t, std::int32_t, std::int64_t, 
// std::float_t, std::double_t, std::string, Symbol, Span
#pragma region atomic_types
namespace atoms 
{
    ...
    class Symbol 
    {
      ...
      public:
        ...
        inline bool operator!=(Symbol const &other) const { return getName() != other.getName(); };
        friend ::std::ostream &operator<<(::std::ostream &out, Symbol const &thing) { return out << thing.getName(); }
    };
    ...
    template <typename Scalar> struct Span 
    {
      private: // state
        using IteratorType =
            std::conditional_t<std::is_same_v<std::remove_const_t<Scalar>, bool>,
      ...
      public: // surface
        ...
        */
        ...
        Span(Span &&other) noexcept : _begin(other._begin), _end(other._end), destructor(std::move(other.destructor))
        {
        ...
        */
        template <typename... Reason> Span<std::remove_const_t<Scalar>> clone(Reason... reason) const &
        {
            ...
            return Span<std::remove_const_t<Scalar>>(std::vector<std::remove_const_t<Scalar>>(_begin, _end));
        }
        ...
        */
        ...
        friend std::ostream &operator<<(std::ostream &stream, Span const &span) { return stream << span.size(); }
    };
} // namespace atoms
...
// robust error handling for argument type mismatches
#pragma region argument_type_mismatch
...
class ArgumentTypeMismatch : public ArgumentTypeMismatch<void> 
{
  public:
    ...
    explicit ArgumentTypeMismatch(VariantType const &variant)
        : ArgumentTypeMismatch<void>([&variant]() {
              ...
              static auto typenames = ::std::map<::std::type_index, char const *>{
                  {typeid(bool), "bool"},       {typeid(int8_t), "char"},          {typeid(int16_t), "short"},
                  {typeid(int32_t), "int"},     {typeid(int64_t), "long"},         {typeid(float_t), "float"},
                  {typeid(double_t), "double"}, {typeid(::std::string), "string"}, {typeid(Symbol), "Symbol"}};
              output << "\", expected "
...
#pragma endregion argument_type_mismatch

// Variant type that includes all atomic expression types, 
// as well as any additional custom atom types
template <typename... AdditionalCustomAtoms>
using AtomicExpressionWithAdditionalCustomAtoms =
    std::variant<bool, std::int8_t, std::int16_t, std::int32_t, std::int64_t, std::float_t, std::double_t, std::string,
                 Symbol, AdditionalCustomAtoms...>;
#pragma endregion atomic_types
...
namespace generic 
{
    ...
    inline constexpr bool isComplexExpression =
        boss::utilities::isInstanceOfTemplate<std::decay_t<T>, ComplexExpressionWithAdditionalCustomAtoms>::value;

    #pragma region Expression_WithAdditionalCustomAtoms
    template <typename... AdditionalCustomAtoms>
    class ExpressionWithAdditionalCustomAtoms
        : public boss::utilities::variant_amend<AtomicExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>,
            ComplexExpressionWithAdditionalCustomAtoms<std::tuple<>, AdditionalCustomAtoms...>>::type 
    {
      public:
        ...
        ExpressionWithAdditionalCustomAtoms &operator=(ExpressionWithAdditionalCustomAtoms const &) = delete;
        ...
        explicit ExpressionWithAdditionalCustomAtoms(T value) noexcept : ExpressionWithAdditionalCustomAtoms(U(value))
        {
        }
            ...
            : SuperType(std::visit(
                boss::utilities::overload(
                    [](ComplexExpressionWithAdditionalCustomAtoms<std::tuple<>, T...> &&unpacked)
                        ...
                        return ComplexExpressionWithAdditionalCustomAtoms<std::tuple<>, AdditionalCustomAtoms...>(
                ...
                (std::add_rvalue_reference_t<typename boss::utilities::variant_amend<
                    ...
                    ComplexExpressionWithAdditionalCustomAtoms<std::tuple<>, T...>>::type>)std::move(other)))
        {
        }
        ...
        ExpressionWithAdditionalCustomAtoms &operator=(ExpressionWithAdditionalCustomAtoms &&) noexcept = default;

        template <typename T>
        std::enable_if_t<
            boss::utilities::isInstanceOfTemplate<std::decay_t<T>, ComplexExpressionWithAdditionalCustomAtoms>::value, bool>
        operator==(T const &other) const
        {
            return std::holds_alternative<
                    ComplexExpressionWithAdditionalCustomAtoms<std::tuple<>, AdditionalCustomAtoms...>>(*this) &&
                (std::get<ComplexExpressionWithAdditionalCustomAtoms<std::tuple<>, AdditionalCustomAtoms...>>(*this) ==
                    other);
        }

        template <typename T>
        std::enable_if_t<
            boss::utilities::isVariantMember<T, AtomicExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>>::value,
            bool>
        operator==(T const &other) const
        {
        ...
        template <typename T>
        std::enable_if_t<!std::is_same_v<T, ExpressionWithAdditionalCustomAtoms>, bool> operator!=(T const &other) const
        {
            return !(*this == other);
        }
        ...
        {
            checkCloneWithoutReason(reason...);
            using ComplexExpression = ComplexExpressionWithAdditionalCustomAtoms<std::tuple<>, AdditionalCustomAtoms...>;
            return std::visit(
                boss::utilities::overload([](auto const &val) -> ExpressionWithAdditionalCustomAtoms { return val; },
        ...
        friend ::std::ostream &operator<<(::std::ostream &out, ExpressionWithAdditionalCustomAtoms const &thing)
        {
    ...
    #pragma endregion Expression_WithAdditionalCustomAtoms

    #pragma region ExpressionArguments_WithAdditionalCustomAtoms
    template <typename... AdditionalCustomAtoms>
        ...
        : public std::vector<ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>> 
    {
      public:
        ...
        template <
            typename... ArgType,
            std::enable_if_t<
                (std::is_convertible_v<ArgType, ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>> && ...),
                bool> = false>
        explicit ExpressionArgumentsWithAdditionalCustomAtoms(ArgType &&...arg)
        {
            (std::vector<ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>>::emplace_back(std::move(arg)), ...);
        }
    };
    #pragma endregion ExpressionArguments_WithAdditionalCustomAtoms

    #pragma region ExpressionSpanArguments_WithAdditionalCustomAtoms
    template <typename... AdditionalCustomAtoms>
    using ExpressionSpanArgumentWithAdditionalCustomAtoms =
        std::variant<Span<bool>, Span<std::int8_t>, Span<std::int16_t>, Span<std::int32_t>, Span<std::int64_t>,
                     Span<std::float_t>, Span<std::double_t>, Span<std::string>, Span<Symbol>,
                     Span<AdditionalCustomAtoms>..., Span<bool const>, Span<std::int8_t const>, Span<std::int16_t const>,
    ...
    class ExpressionSpanArgumentsWithAdditionalCustomAtoms
        : public std::vector<ExpressionSpanArgumentWithAdditionalCustomAtoms<AdditionalCustomAtoms...>> 
    {
      public:
        using std::vector<ExpressionSpanArgumentWithAdditionalCustomAtoms<AdditionalCustomAtoms...>>::vector;

        template <typename... ArgType,
                std::enable_if_t<(std::is_convertible_v<ArgType, ExpressionSpanArgumentWithAdditionalCustomAtoms<
                                                                    AdditionalCustomAtoms...>> &&
                                    ...),
                                bool> = false>
        explicit ExpressionSpanArgumentsWithAdditionalCustomAtoms(ArgType &&...arg)
        {
            ...
            (std::vector<ExpressionSpanArgumentWithAdditionalCustomAtoms<AdditionalCustomAtoms...>>::emplace_back(
                std::move(arg)),
            ...);
        }
        ...
        ExpressionSpanArgumentsWithAdditionalCustomAtoms(ExpressionSpanArgumentsWithAdditionalCustomAtoms const &) = delete;
        ...
        ExpressionSpanArgumentsWithAdditionalCustomAtoms(ExpressionSpanArgumentsWithAdditionalCustomAtoms &&) noexcept =
            default;
        ExpressionSpanArgumentsWithAdditionalCustomAtoms &
    ...
    #pragma endregion ExpressionSpanArguments_WithAdditionalCustomAtoms

    // unified (dynamically-typed, visitor-based) interface for all types
    #pragma region ArgumentWrapper
    ...
    */
    template <class T> 
    class MovableReferenceWrapper 
    {
      public:
        typedef T type;

        explicit MovableReferenceWrapper(std::reference_wrapper<T> &&ref) : _ptr(std::addressof(ref.get())) { ; }
        ...
        template <typename T2>
        constexpr std::enable_if_t<boss::utilities::is_comparable<T, T2>::value, bool> operator==(T2 const other) const
        {
            return *_ptr == other;
        }
    ...
    using ArgumentWrappeeType = typename boss::utilities::variant_amend<
        typename boss::utilities::rewrap_variant_arguments<
            MovableReferenceWrapper, AtomicExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>>::type,
    ...
    using ConstArgumentWrappeeType = typename boss::utilities::variant_amend<
        typename utilities::rewrap_variant_arguments<
            MovableReferenceWrapper, typename utilities::make_variant_members_const<
                                        AtomicExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>>::type>::type,
        std::vector<bool>::const_reference,
        MovableReferenceWrapper<ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...> const>>::type;

    template <bool ConstWrappee, typename... AdditionalCustomAtoms> 
    class ArgumentWrapper 
    {
      public:
        using WrappeeType = std::conditional_t<ConstWrappee, ConstArgumentWrappeeType<AdditionalCustomAtoms...>,
      ...
      public:
        ...
        {
            return std::visit(
                [](auto &&argument) { return ArgumentWrapper<true, AdditionalCustomAtoms...>(argument.get()); }, argument);
        };
            ...
            ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>() &&
        {
            return std::move(std::visit(
                [](auto &&expression) -> ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...> {
                    ...
                    else if constexpr (::std::disjunction_v<::std::is_same<::std::decay_t<decltype(expression)>,
                                                                        ::std::vector<bool>::reference>,
        ...
        */
        template <typename T,
                typename = std::enable_if_t<std::conjunction<
                    std::negation<boss::utilities::isVariantMember<MovableReferenceWrapper<const T>, WrappeeType>>,
        ...
        template <typename T,
                typename = std::enable_if_t<std::disjunction_v<std::is_same<T, std::vector<bool>::const_reference>,
        ...
        template <typename... Reason> auto clone(Reason... reason) const
        {
            ...
            static auto unwrap = [reason...](auto const &refWrapper) {
                if constexpr (boss::utilities::isInstanceOfTemplate<std::decay_t<decltype(refWrapper)>,
            ...
            return std::visit(
                [reason...](auto const &typedArg) -> ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...> {
                    if constexpr (boss::utilities::isInstanceOfTemplate<std::decay_t<decltype(typedArg)>,
                                                                        ExpressionWithAdditionalCustomAtoms>::value) {
                        return typedArg.get().clone(reason...);
                    }
                    else if constexpr (boss::utilities::isInstanceOfTemplate<std::decay_t<decltype(typedArg)>,
                                                                            MovableReferenceWrapper>::value) {
            ...
            return visit(
                [&stream](auto &&val) -> auto & {
                    if constexpr (::std::disjunction_v<
                                    ::std::is_same<::std::decay_t<decltype(val)>, ::std::vector<bool>::reference>,
    ...
    decltype(auto) visit(Func &&func, ArgumentWrapper<ConstWrappee, AdditionalCustomAtoms...> const &wrapper)
    {
        return visit(
            [&](auto &&unwrapped) {
                if constexpr (boss::utilities::isInstanceOfTemplate<::std::decay_t<decltype(unwrapped)>,
                                                                    MovableReferenceWrapper>::value) {
                    if constexpr (::std::is_same_v<::std::remove_cv_t<::std::remove_reference_t<decltype(unwrapped.get())>>,
                                                ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>>) {
                ...
                else if constexpr (::std::is_same_v<::std::remove_cv_t<::std::remove_reference_t<decltype(unwrapped)>>,
                                                    ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>>) {
    ...
    template <typename Func, auto ConstWrappee, typename... AdditionalCustomAtoms>
    decltype(auto) visit(Func &&func, ArgumentWrapper<ConstWrappee, AdditionalCustomAtoms...> &&wrapper)
    {
        return visit(
            [&](auto &&unwrapped) {
                if constexpr (boss::utilities::isInstanceOfTemplate<::std::decay_t<decltype(unwrapped)>,
    ...
    namespace utilities 
    {
        ...
        */
        ...
        template <typename... T> struct isConstArgumentWrapperType<ArgumentWrapper<true, T...>> : public std::true_type {
        };
        template <typename... T> inline constexpr bool isConstArgumentWrapper = isConstArgumentWrapperType<T...>::value;
    } // namespace utilities

    template <typename StaticArgumentsContainer, bool IsConstWrapper = false, typename... AdditionalAtoms>
    class ExpressionArgumentsWithAdditionalCustomAtomsWrapper 
    {
        ...
        using DynamicArgumentsContainer =
            std::conditional_t<IsConstWrapper, ExpressionArgumentsWithAdditionalCustomAtoms<AdditionalAtoms...> const,
        ...
        using SpanArgumentsContainer =
            std::conditional_t<IsConstWrapper, ExpressionSpanArgumentsWithAdditionalCustomAtoms<AdditionalAtoms...> const,
      ...
      public:
        ...
        size_t size() const
        {
            return std::tuple_size_v<StaticArgumentsContainer> + arguments.size() +
                std::accumulate(spanArguments.begin(), spanArguments.end(), 0, [](auto soFar, auto &thisOne) {
        ...
        struct Iterator 
        {
            ...
            ArgumentWrapper<IsConstIterator, AdditionalAtoms...> operator*() const { return container.at(index); }
            ...
            Iterator &operator=(Iterator &&other) noexcept
            {
                ...
                new (&container) decltype(container)(other.container.staticArguments, other.container.arguments,
                                                    other.container.spanArguments);
                return *this;
            }
            ...
            Iterator(std::conditional_t<IsConstIterator, ExpressionArgumentsWithAdditionalCustomAtomsWrapper const,
                                        ExpressionArgumentsWithAdditionalCustomAtomsWrapper>
                        container,
                    size_t index)
        ...
        template <size_t... I>
        constexpr ArgumentWrapper<IsConstWrapper, AdditionalAtoms...> getStaticArgument(size_t index) const
        {
            return getStaticArgument(index, std::make_index_sequence<std::tuple_size_v<StaticArgumentsContainer>>());
        }
        ...
        ArgumentWrapper<IsConstWrapper, AdditionalAtoms...> operator[](size_t index) const
        {
            if constexpr ((std::tuple_size_v<StaticArgumentsContainer>) > 0) {
                ...
                for (auto &spanArgument : spanArguments) {
                    ...
                    argumentPrefixScan += std::visit([](auto &&spanArgument) { return spanArgument.size(); }, spanArgument);
                }
            }
      #if defined(_MSC_VER)
            __assume(0);
        ...
        ArgumentWrapper<IsConstWrapper, AdditionalAtoms...> getSpanArgument(size_t index) const
        {
            ...
            for (auto &spanArgument : spanArguments) {
                if (index >= argumentPrefixScan &&
                    index < argumentPrefixScan + std::visit([](auto &span) { return span.size(); }, spanArgument)) {
                    return std::visit(
                        [&](auto &&spanArgument) -> ArgumentWrapper<IsConstWrapper, AdditionalAtoms...> {
                            if constexpr ((!IsConstWrapper && std::is_same_v<std::decay_t<decltype(spanArgument.at(0))>,
                                                                            std::vector<bool>::const_reference>) ||
                                        ((std::is_const_v<std::remove_reference_t<decltype(spanArgument)>> ||
                                            std::is_const_v<
                                                std::remove_reference_t<decltype(spanArgument.at(0))>>)&&!IsConstWrapper)) {
                                throw std::runtime_error("cannot convert const span to non-const argument");
                            }
                            else if constexpr (

                                std::is_same_v<std::decay_t<decltype(spanArgument.at(0))>, std::vector<bool>::reference> ||
                                std::is_same_v<std::decay_t<decltype(spanArgument.at(0))>,
                                            std::vector<bool>::const_reference>) {
                                if constexpr (IsConstWrapper || std::is_same_v<std::decay_t<decltype(spanArgument.at(0))>,
                                                                            std::vector<bool>::const_reference>) {
                                    return std::vector<bool>::const_reference(spanArgument.at(index - argumentPrefixScan));
                                }
                                else {
        ...
        */
            ...
            ExpressionArgumentsWithAdditionalCustomAtoms<AdditionalAtoms...>() &&
        {
            ...
            ExpressionArgumentsWithAdditionalCustomAtoms<AdditionalAtoms...> result;
            result.reserve(this->size());
            std::transform(std::make_move_iterator(std::begin(*this)), std::make_move_iterator(std::end(*this)),
    ...
    #pragma endregion ArgumentWrapper

    #pragma region ComplexExpression_WithAdditionalCustomAtoms
    // NOLINTBEGIN(bugprone-exception-escape)
    // see https://github.com/llvm/llvm-project/issues/54668
    template <typename StaticArgumentsTuple, typename... AdditionalCustomAtoms>
    class ComplexExpressionWithAdditionalCustomAtoms 
    {
      ...
      public:
        ...
        {
            return {(std::get<std::remove_reference_t<typename std::tuple_element<I, StaticArgumentsTuple>::type>>(
                arguments.at(I)))...};
        }

        template <typename T>
        void cloneIfNecessary(ExpressionArgumentsWithAdditionalCustomAtoms<AdditionalCustomAtoms...> &result,
                            ComplexExpressionWithAdditionalCustomAtoms<T, AdditionalCustomAtoms...> const &e) const
        {
            result.push_back(e.clone());
        }

        template <typename T, typename = std::enable_if_t<boss::utilities::isVariantMember<
                                T, AtomicExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>>::value>>
        void cloneIfNecessary(ExpressionArgumentsWithAdditionalCustomAtoms<AdditionalCustomAtoms...> &result, T e) const
        {
        ...
        */
        std::tuple<Symbol, StaticArgumentsTuple, ExpressionArgumentsWithAdditionalCustomAtoms<AdditionalCustomAtoms...>,
                ExpressionSpanArgumentsWithAdditionalCustomAtoms<AdditionalCustomAtoms...>>
        decompose() &&
        {
            return {std::move(head), std::move(staticArguments), std::move(arguments), std::move(spanArguments)};
        }
        ...
        ComplexExpressionWithAdditionalCustomAtoms(
            ...
            ExpressionArgumentsWithAdditionalCustomAtoms<AdditionalCustomAtoms...> &&arguments = {},
            ExpressionSpanArgumentsWithAdditionalCustomAtoms<AdditionalCustomAtoms...> &&spanArguments = {})
            : head(std::move(head)), staticArguments(std::move(staticArguments)), arguments(std::move(arguments)),
            spanArguments(std::move(spanArguments))
        {
        }
        ...
        explicit ComplexExpressionWithAdditionalCustomAtoms(
            Symbol const &head, ExpressionArgumentsWithAdditionalCustomAtoms<AdditionalCustomAtoms...> &&arguments)
            : ComplexExpressionWithAdditionalCustomAtoms(
                head, convertToTuple(arguments, std::make_index_sequence<std::tuple_size<StaticArgumentsTuple>::value>()),
                {std::move_iterator(next(begin(arguments), std::tuple_size<StaticArgumentsTuple>::value)),
                std::move_iterator(end(arguments))}){};

        template <typename = std::enable_if<std::tuple_size<StaticArgumentsTuple>::value == 0>>
        explicit ComplexExpressionWithAdditionalCustomAtoms(
            Symbol &&head, ExpressionArgumentsWithAdditionalCustomAtoms<AdditionalCustomAtoms...> &&arguments)
            : ComplexExpressionWithAdditionalCustomAtoms(
                std::move(head),
                convertToTuple(arguments, std::make_index_sequence<std::tuple_size<StaticArgumentsTuple>::value>()),
                {std::move_iterator(next(begin(arguments), std::tuple_size<StaticArgumentsTuple>::value)),
                std::move_iterator(end(arguments))}){};
        ...
        {
            return std::move(ComplexExpressionWithAdditionalCustomAtoms< // NOLINT(hicpp-explicit-conversions)
                            std::tuple<>, AdditionalCustomAtoms...>(
                head,
                convertStaticToDynamicArguments(std::make_index_sequence<std::tuple_size<StaticArgumentsTuple>::value>())));
        }

        template <typename = std::enable_if<sizeof...(AdditionalCustomAtoms) != 0>, typename OtherTuple, typename... T>
        explicit ComplexExpressionWithAdditionalCustomAtoms(
            ...
            : head(other.getHead())
        {
            ...
            // move statics
            std::apply([this](auto &&...staticArgs) { (arguments.emplace_back(std::move(staticArgs)), ...); },
            ...
            for (auto &&arg : otherDynamics) {
                visit(boss::utilities::overload(
                        [this](ComplexExpressionWithAdditionalCustomAtoms<OtherTuple, T...> &&typedArg) {
                            arguments.emplace_back(ComplexExpressionWithAdditionalCustomAtoms(std::move(typedArg)));
                        },
                        [this](auto &&typedArg) { arguments.emplace_back(std::forward<decltype(typedArg)>(typedArg)); }),
                    std::move(arg));
            }
            spanArguments.reserve(otherSpans.size());
            for (auto &&span : otherSpans) {
                std::visit(
                    [this](auto &&typedSpan) { spanArguments.emplace_back(std::forward<decltype(typedSpan)>(typedSpan)); },
                    std::move(span));
            }
        }

        ExpressionArgumentsWithAdditionalCustomAtomsWrapper<decltype(staticArguments), false, AdditionalCustomAtoms...>
        getArguments()
        {
            return {staticArguments, arguments, spanArguments};
        }
        ExpressionArgumentsWithAdditionalCustomAtomsWrapper<decltype(staticArguments) const, true, AdditionalCustomAtoms...>
        getArguments() const
        {
            return {staticArguments, arguments, spanArguments};
        }

        ExpressionArgumentsWithAdditionalCustomAtoms<AdditionalCustomAtoms...> const &getDynamicArguments() const &
        {
            return arguments;
        };
        ...
        ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...> getArgument(size_t index) &&
        {
            return visit(
                ...
                std::forward<decltype(getArguments().at(index).getArgument())>(getArguments().at(index).getArgument()));
        }

        template <typename... Reason>
        ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...> cloneArgument(size_t index, Reason... reason) const
        {
            checkCloneWithoutReason(reason...);
            return visit(
                [reason...](auto const &unwrapped) -> ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...> {
                    if constexpr (boss::utilities::isInstanceOfTemplate<::std::decay_t<decltype(unwrapped)>,
                                                                        MovableReferenceWrapper>::value) {
                        if constexpr (::std::is_same_v<
                                        ::std::remove_cv_t<::std::remove_reference_t<decltype(unwrapped.get())>>,
                            ...
                            return visit(
                                boss::utilities::overload(
                                    ...
                                    [](auto const &arg) -> ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...> {
                                        return arg;
                                    }),
                                unwrapped.get());
                        }
                            ...
                            return unwrapped.get();
                                                        ...
                                                        ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>>) {
                        return visit(
                            [reason...](auto const &arg) -> ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...> {
                                return arg.clone(reason...);
                            },
                            unwrapped);
                    }
        ...
        bool operator==(ComplexExpressionWithAdditionalCustomAtoms<std::tuple<StaticArgumentTypes...>,
        ...
        bool operator!=(ComplexExpressionWithAdditionalCustomAtoms const &other) const { return !(*this == other); }

        template <typename... Reason> ComplexExpressionWithAdditionalCustomAtoms clone(Reason... reason) const
        {
            checkCloneWithoutReason(reason...);
            ...
            for (auto &it : spanArguments) {
                newSpanArguments.push_back(std::visit(
                    [&](auto const &v) -> ExpressionSpanArgumentWithAdditionalCustomAtoms<AdditionalCustomAtoms...> {
                        return v.clone(reason...);
                    },
                    it));
            }

            return ComplexExpressionWithAdditionalCustomAtoms(head, {}, std::move(copiedArgs), std::move(newSpanArguments));
        }
        ...
        * Expression
        */
        friend ::std::ostream &operator<<(::std::ostream &out, ComplexExpressionWithAdditionalCustomAtoms const &e)
        {
            out << e.getHead() << "[";
        ...
        ComplexExpressionWithAdditionalCustomAtoms(ComplexExpressionWithAdditionalCustomAtoms const &) = delete;
        ComplexExpressionWithAdditionalCustomAtoms &operator=(ComplexExpressionWithAdditionalCustomAtoms const &) = delete;
    };
    // NOLINTEND(bugprone-exception-escape)
    #pragma endregion ComplexExpression_WithAdditionalCustomAtoms

    #pragma region ExtensibleExpressionSystem
    template <typename... AdditionalCustomAtoms> 
    class ExtensibleExpressionSystem 
    {
      public:
        ...
        using ComplexExpressionWithStaticArguments =
            ComplexExpressionWithAdditionalCustomAtoms<std::tuple<StaticArgumentTypes...>, AdditionalCustomAtoms...>;
        ...
        using ExpressionArguments = ExpressionArgumentsWithAdditionalCustomAtoms<AdditionalCustomAtoms...>;
        using ExpressionSpanArguments = ExpressionSpanArgumentsWithAdditionalCustomAtoms<AdditionalCustomAtoms...>;
        using ExpressionSpanArgument = ExpressionSpanArgumentWithAdditionalCustomAtoms<AdditionalCustomAtoms...>;
    };
    #pragma endregion ExtensibleExpressionSystem

    // evaluate (by reference, by value, or a pointer to value)
    #pragma region type_safe_getters
    ...
    T &get(generic::ArgumentWrapper<ConstWrappee, AdditionalCustomAtoms...> const &wrapper)
    {
        try {
            return ::std::visit(
                [](auto &argument) -> T & {
                    if constexpr (::std::is_same_v<::std::decay_t<decltype(argument)>, MovableReferenceWrapper<T>>) {
                        return argument.get();
                    }
                    ...
                    else if constexpr (boss::utilities::isInstanceOfTemplate<::std::decay_t<decltype(argument)>,
                                                                            MovableReferenceWrapper>::value) {
                        if constexpr (boss::utilities::isInstanceOfTemplate<::std::decay_t<decltype(argument.get())>,
                                                                            ExpressionWithAdditionalCustomAtoms>::value) {
    ...
    template <typename T, auto ConstWrappee, typename... AdditionalCustomAtoms>
    bool holds_alternative(generic::ArgumentWrapper<ConstWrappee, AdditionalCustomAtoms...> const &wrapper)
    {
        return ::std::visit(
            [](auto &argument) {
                if constexpr (::std::is_same_v<::std::decay_t<decltype(argument)>, MovableReferenceWrapper<T>>) {
                ...
                else if constexpr (::std::is_same_v<::std::decay_t<decltype(argument)>, ::std::vector<bool>::reference>) {
                    if constexpr (::std::is_same_v<::std::decay_t<T>, ::std::vector<bool>::reference>) {
    ...
    #pragma endregion type_safe_getters
...
#pragma region AdditionalCustomAtoms

namespace std 
{
    template <typename... AdditionalCustomAtoms>
    struct variant_size<typename boss::expressions::generic::ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>>
    ...
    template <typename... AdditionalCustomAtoms>
    struct variant_size<
        const typename boss::expressions::generic::ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>>
    ...
    struct variant_alternative<
        I, typename boss::expressions::generic::ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>>
        : variant_alternative<I, typename boss::expressions::generic::ExpressionWithAdditionalCustomAtoms<
                                    AdditionalCustomAtoms...>::SuperType> {
    };
    template <typename Func, typename... AdditionalCustomAtoms>
    decltype(auto)
    visit(Func &&func,
        typename boss::expressions::generic::ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...> &e)
    {
        return visit(
            ::std::forward<Func>(func),
            (typename boss::expressions::generic::ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...>::SuperType
                &)e);
    };
    template <typename Func, typename... AdditionalCustomAtoms>
    decltype(auto)
    visit(Func &&func,
        typename boss::expressions::generic::ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...> const &e)
    {
        return visit(::std::forward<Func>(func), (typename boss::expressions::generic::ExpressionWithAdditionalCustomAtoms<
        ...
        typename boss::expressions::generic::ExpressionWithAdditionalCustomAtoms<AdditionalCustomAtoms...> &&e)
    {
        return visit(::std::forward<Func>(func),
                    (std::add_rvalue_reference_t<typename boss::expressions::generic::ExpressionWithAdditionalCustomAtoms<
    ...
    struct _Extra_visit_slot_needed<::std::__detail::__variant::__deduce_visit_result<void>,
...
#pragma endregion AdditionalCustomAtoms

#pragma region ExtensibleExpressionBuilder
namespace boss::utilities 
{

template <typename ExpressionSystem = DefaultExpressionSystem> 
class ExtensibleExpressionBuilder 
{
    Symbol const s;
  ...
  public:
     ...
     * it explicitly
     */
    typename ExpressionSystem::Expression convertConstCharToStringAndOnToExpression(char const *v) const
    {
        return ::std::string((char const *)v);
    }
    template <typename T> typename ExpressionSystem::Expression convertConstCharToStringAndOnToExpression(T &&v) const
    {
    ...
    template <typename Ts> using isStaticArgument = ::std::disjunction<isComplexExpression<Ts>, isAtom<Ts>>;
     ...
     */
    template <typename... Ts>
    ::std::enable_if_t<(sizeof...(Ts) > 0) && !(std::conjunction_v<isSpanArgument<::std::decay_t<Ts>>...>)&&!(
    ...
    {
        auto spans = std::array{std::move(args)...}; // unfortunately, vectors cannot be initialized with move-only
...
static ExpressionBuilder operator""_(const char *name, size_t /*unused*/) { return ExpressionBuilder(name); };

} // namespace boss::utilities

#pragma endregion ExtensibleExpressionBuilder